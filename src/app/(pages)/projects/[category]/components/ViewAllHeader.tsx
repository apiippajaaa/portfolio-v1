...
export default function ViewAllHeader({ title, description, category }: Props) {
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden mx-auto w-full max-w-6xl">
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col justify-center">
        ...
        <div className="flex flex-col items-center text-center lg:hidden">
          <div className="mb-6 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/3 px-5 py-2 text-[10px] uppercase tracking-[0.35em] text-white/45 backdrop-blur-2xl">
          ...
          {category === "development" && (
            <Link
              ...
              className="group mt-10 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/3 px-5 py-3 text-sm font-medium text-white/70 backdrop-blur-xl transition-all duration-300 hover:border-yellow-400/20 hover:bg-yellow-400 hover:text-black"
            >
              Archive
              <ArrowUpRight
                size={16}
        ...
        <div className="hidden lg:block">
          <div className="mb-8 inline-flex w-fit items-center gap-3 rounded-full border border-white/10 bg-white/3 px-5 py-2 text-[11px] uppercase tracking-[0.35em] text-white/45 backdrop-blur-2xl">
            <div className="relative flex h-2 w-2 items-center justify-center">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-yellow-400/60" />

              <span className="absolute inline-flex h-3 w-3 rounded-full bg-yellow-400/20 blur-xs" />
          ...
          <div className="grid items-end gap-10 lg:grid-cols-[1fr_auto]">
            ...
            {category === "development" && (
              <div className="flex justify-end">
                <Link
                  href="/projects/development/archive"
                  className="group inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/3 px-5 py-3 text-sm font-medium text-white/70 backdrop-blur-xl transition-all duration-300 hover:border-yellow-400/20 hover:bg-yellow-400 hover:text-black"
                >
                  Archive
                  <ArrowUpRight
                    size={16}
